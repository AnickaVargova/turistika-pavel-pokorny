    ...
    seradClanky() {
      if (this.$route.name === "Vypraveni") {
        const parseDateToYMD = (str) =>
          str
            ? str?.slice(6) + "-" + str?.slice(3, 5) + "-" + str?.slice(0, 2)
            : "";

        this.clankyPodKategorie.map((item) => {
          if (item.pridano) {
            item.timestamp = Date.parse(parseDateToYMD(item.pridano?.trim()));
          } else {
            item.timestamp = 0;
          }
        });

        this.clankyPodKategorie.sort((a, b) => {
          if (b.datum.trim().slice(-4) !== a.datum.trim().slice(-4)) {
            return (
